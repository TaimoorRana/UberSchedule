class SequenceBuilderController < ApplicationController
  ...
  def sequence_builder
    ...
    preferences.exists?(14) ? allowSummer = true : allowSummer = flase
    ...
    until accumulated_credits > max_credits do
     ...
     @semester_sequence.push(semester_year)
      previous_course_id = 0
      semester_section[current_semester].each do |section| #loop through all courses given this semester
        course_id = section.course_id
        if(previous_course_id != course_id) and !completed_courses.include?(course_id)
          previous_course_id = course_id
          missing_prereq = 0
          course = Course.find(course_id) #TODO DB CALL TO BE REMOVED
        ...
        until classes_counter == max_classes do
          ...
          if !(used_index.include? random_index)
            ...
            accumulated_credits = accumulated_credits.to_i + added_course.credit.to_i
